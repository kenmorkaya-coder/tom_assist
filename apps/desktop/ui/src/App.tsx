...
type View = "Overview" | "Ledger" | "Interventions" | "Audit" | "Settings" | "Diagnostics" | "Exchange";
const views: View[] = ["Overview", "Ledger", "Interventions", "Audit", "Settings", "Diagnostics", "Exchange"];
...
export function App({ backend = tauriBackend }: { backend?: DesktopBackend }) {
  ...
  return <div class="app-shell">
    ...
    <main>
      ...
      {view === "Settings" && <section class="settings"><label>Project name<input aria-label="Project name" value={projectName} onInput={(event) => setProjectName(event.currentTarget.value)}/></label><button disabled={!active || !projectName.trim()} onClick={() => active && void run(async () => { await backend.rename(active, projectName); })}>Rename project</button><label>Packet budget<input aria-label="Packet budget" type="number" value="500" min="1" max="1200"/></label><label>Retention<select><option>State-focused</option><option>Full local</option><option>Ephemeral transcript</option></select></label><button disabled={!active} onClick={() => active && void run(async () => { await backend.archive(active); })}>Archive project</button></section>}
      {view === "Settings" && <Recovery projectId={active?.id} backend={backend} onImported={project => refresh(project.id)}/>}
      {view === "Settings" && active && <MemoryControls key={active.id} projectId={active.id} backend={backend}/>}
      {view === "Exchange" && active && <Exchange key={active.id} projectId={active.id} backend={backend}/>}
