...
class NativeExtensionService implements ExtensionService {
  ...
  private async evaluateWithAcceptance(projectId: string, turn: ProviderTurn, packetDigest: string, accept: boolean): Promise<EvaluationBadge> { const result = await this.call("response.evaluate", projectId, { project_id: projectId, packet_digest: packetDigest, response_turn_id: turn.contentHash, response_text: turn.normalizedText, ordinal: turn.ordinal, complete: turn.complete, created_at: now(), accept_for_experience: accept }); return { result: result.result, count: result.intervention_ids?.length ?? 0, severity: result.result === "CONFLICT" ? "blocking" : result.result === "REVIEW" ? "warning" : "info" }; }
  async evaluate(projectId: string, turn: ProviderTurn, packetDigest: string): Promise<EvaluationBadge> { return this.evaluateWithAcceptance(projectId, turn, packetDigest, false); }
  async accept(projectId: string, turn: ProviderTurn, packetDigest: string): Promise<EvaluationBadge> { return this.evaluateWithAcceptance(projectId, turn, packetDigest, true); }
