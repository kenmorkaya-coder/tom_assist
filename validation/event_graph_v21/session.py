"""Matched 96-update event-evidence granularity pilot; outputs belong on Passport."""
...
from validation.event_graph_v20 import session as previous
...
from gateway.event_graph_span_wire import to_wire, source_tokens, materialize_span
...
RUN = Path('/Volumes/My Passport for Mac/gemmatraining/event-graph-lora-v21-pilot-001')
PLAN = dict(updates_per_arm=96, seed=151, learning_rate=0.000005, baseline_steps=4380,
    arms=['event_evidence'], changed_records=2, changed_fields=4,
    optimizer='reset as in V20', variable='only four event-evidence fields in the same two revision examples',
    comparison='completed V20 revision_replay, same sources, names, semantic fields, other evidence, 94 other examples, starting weights, sample order and settings',
    retention='original132 all exact; prior156 at least155; dev192 at least191 exact and all192 valid',
    evidence='repair all eight shared revision failures; diagnostic at least50/56 and all four original anchors exact; inspect boundary errors against V20',
    selection='retain4380 regardless; review before any further run or promotion',
    limitation='one seed, exposed diagnostic; four valid evidence fields narrowed as a bundle, not individual field attribution; no held-out claim')
EVIDENCE = {'v13-train-revision-0-0': ((0,17),(17,43)),
            'v13-train-revision-0-1': ((0,15),(9,37))}


def training_rows(arm):
    assert arm == 'event_evidence'
    rows = previous.training_rows('revision_replay')
    for row in rows:
        origin = row.get('source_row_id')
        if origin not in EVIDENCE: continue
        tokens = source_tokens(row['source'])
        for event, (start,end) in zip(row['graph']['events'], EVIDENCE[origin]):
            event['evidence'] = [materialize_span({'token_start':start,'token_end':end},row['source'],tokens)]
...
def freeze(run):
    ...
    files = [Path(__file__).resolve(), ROOT/'gateway/tests/test_event_graph_v21.py',
        ROOT/'validation/event_graph_v20/session.py', previous.RUN/'freeze.json',
        previous.RUN/'revision_replay/report.json', previous.RUN/'revision_replay/predictions.jsonl',
        previous.RUN/'revision_replay/training_complete.json', previous.RUN/'revision_replay/training_started.json',
        previous.RUN/'revision_replay/sampled_ids.jsonl',
    ...
    frozen = {'created_unix':time.time(),'plan':PLAN,'files':{str(p):old.prior.sha(p) for p in files},
        'thresholds':old.verify(V16)['thresholds'],'training_ids':[r['id'] for r in training_rows('event_evidence')],
        'changed_ids':[a['id'] for a,b in zip(previous.training_rows('revision_replay'),training_rows('event_evidence')) if a!=b],
        'replacement_mapping':{r['id']:r['source_row_id'] for r in training_rows('event_evidence') if 'source_row_id' in r}}
...
def train(run, arm):
    ...
    control_receipt=json.loads((previous.RUN/'revision_replay/training_complete.json').read_text())
