"""One fresh held-out pass for the development-selected 3300-step adapter."""
...
from validation.event_graph_v11.session import verify as verify_session, adapter
...
SESSION=ROOT/'.tmp/event-graph-lora-v11-session-001'
...
def freeze(run):
    ...
    result_path=ROOT/'validation/runs/event-graph-lora-v11-session-001/RESULT.json'
    result=json.loads(result_path.read_text())
    assert result['comparison']['selected_epoch']==1
    assert result['comparison']['selected_cumulative_steps']==3300
    assert result['reports'][1]['development_thresholds_met']
    selected=adapter(SESSION,1)
    assert sha(selected/'adapters.safetensors')=='4d5201649f27778c53db424a3b0631d6434d3ef017085d1314b0387b43d68e23'
    ...
    files += [ROOT/'gateway/tests/test_event_graph_v12.py',result_path]
    write_new(run/'freeze.json',{'created_unix':time.time(),'selected_cumulative_steps':3300,
