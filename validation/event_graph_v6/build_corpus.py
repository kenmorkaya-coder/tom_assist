...
def make_row(family, index, variant):
    ...
    site = f'{("Mica", "Juniper")[index % 2]} {number:02d}-{index+1}'
    a, b = f'{site} excavation', f'{site} vibration'
    c, d = f'{site} superintendent', f'{site} discharge'
    value = ('3.83', '8.47')[index % 2]
    higher = str(Decimal(value) + Decimal('1.19'))
    g = None
    if family in ('quantity', 'comparator', 'polarity'):
        ...
        person = c + (' deputy' if changed else '')
        ...
        qtext=f'{c} is below 1.25 m'; rtext=f'{d} is above 4.75 m'
        ...
        q=g.predicate(low,'1.25','m','LT',qtext)
        if family=='nested':cond=g.condition(operator,[p,q])
        else:
            high=g.entity(d); r=g.predicate(high,'4.75','m','GT',rtext)
        ...
        old=f'DOC-{number}-{index}-V7';new=f'DOC-{number}-{index}-V'+('9' if changed else '8')
        ...
        date=f'2031-04-{11+index+(1 if changed else 0):02d}'
        ...
        cap=('11.6','25.4')[index]
        if changed:cap=str(Decimal(cap)+Decimal('7.3'))
        ...
        value=('-1.37','0','2.63','5.81','9.17','12.43')[index]
    ...
    return {'id':f'v6-{family}-{index}-{variant}','group':f'v6-{family}-{index}',
