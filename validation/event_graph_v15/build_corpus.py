...
from validation.event_graph_v15.wording import render
import json
...
def make_row(family, index, variant):
    ...
    site = f'{("Kestrel Viaduct", "Sorrel Basin")[index % 2]} {number:02d}-{index+1}'
    a, b = f'{site} excavation', f'{site} vibration'
    c, d = f'{site} superintendent', f'{site} discharge'
    value = ('3.07', '11.29')[index % 2]
    higher = str(Decimal(value) + Decimal('2.31'))
    g = None
    if family in ('quantity', 'comparator', 'polarity'):
        ...
        qtext=f'{c} is below 1.91 m'; rtext=f'{d} is above 6.39 m'
        ...
        q=g.predicate(low,'1.91','m','LT',qtext)
        if family=='nested':cond=g.condition(operator,[p,q])
        else:
            high=g.entity(d); r=g.predicate(high,'6.39','m','GT',rtext)
        ...
        old=f'DRAW-{number}-{index}-V7';new=f'DRAW-{number}-{index}-V'+('9' if changed else '8')
        ...
        date=f'2036-09-{11+index+(1 if changed else 0):02d}'
        ...
        cap=('46.7','83.2')[index]
        if changed:cap=str(Decimal(cap)+Decimal('9.4'))
        ...
        value=('-3.41','0','4.93','7.67','12.59','17.23')[index]
    ...
    return {'id':f'v15-{family}-{index}-{variant}','group':f'v15-{family}-{index}',
