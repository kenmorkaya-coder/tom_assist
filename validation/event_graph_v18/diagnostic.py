...
INTERVENTIONS = ('anchor', 'rename_site', 'distinct_prefixes', 'short_names', 'wording')


def rows():
    result = []
    for i, site in enumerate(('Marsh Gallery D00-K', 'Flint Tunnel D01-K', 'Flint Tunnel D03-K', 'Unit Delta')):
        suffixes = ('ground settlement', 'groundwater pumping') if i < 3 else ('flow', 'flow restriction')
        original_a, original_b = [f'{site} {suffix}' for suffix in suffixes]
        original_form = 'The process bringing about {t} is {s}.' if i < 3 else '{s} brings about {t}.'
        for name in INTERVENTIONS:
            a, b = original_a, original_b
            canonical = original_form
            if name == 'rename_site':
                a, b = [f'Silver Court R{i:02d}-M {suffix}' for suffix in suffixes]
            elif name == 'distinct_prefixes':
                b = f'Silver Court R{i:02d}-M {suffixes[1]}'
            elif name == 'short_names':
                a, b = f'Process A{i}', f'Process B{i}'
            elif name == 'wording':
                # Preserve entity order, changing only the causal expression.
                canonical = '{t} is caused by {s}.' if i < 3 else '{s} causes {t}.'
            paraphrase = '{t} results from {s}.'
            for v in range(3):
                ...
                result.append({'id': f'v18-{name}-{i}-{v}', 'group': f'v18-{name}-{i}', 'anchor_index': i,
...
def freeze(run):
    ...
    parent.prior.write_new(run / 'freeze.json', {
        ...
        'files': {str(p): parent.prior.sha(p) for p in (Path(__file__).resolve(), HERE / 'PROTOCOL.md',
            ROOT / 'gateway/tests/test_event_graph_v18.py', run / 'diagnostic.jsonl',
            ROOT / 'validation/runs/event-graph-lora-v16-session-001/TRANSITIONS.json',
            ROOT / 'validation/runs/event-graph-lora-v17-diagnostic-001/RESULT.json')},
