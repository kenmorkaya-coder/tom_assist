"""Bounded lower-rate continuation from the selected v14 adapter."""
...
from validation.event_graph_v14 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v14-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 792, "seeds": [151],
        "learning_rate": 0.000005, "baseline_steps": 4380,
        "initialization": "selected v14 epoch 1 weights; optimizer reset",
        "selection": "retain original 132 exact and prior 156 score of at least 155 first; then total exact, valid, fewer invented authorities, paraphrase, contrast, earliest"}
...
def freeze(run):
    ...
    assert preflight['train']['count']==1584 and preflight['valid']['count']==192
    ...
    paths.append(ROOT/'gateway/tests/test_event_graph_v16.py')
    ...
    prior.write_new(run/'freeze.json',{'created_unix':time.time(),'plan':PLAN,
        ...
        'parent_artifacts':{n:prior.sha(PARENT/n) for n in names},'development_records':192})
...
def train(run, epoch):
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 792, "save_every": 792,
...
def development_score(rows, predictions, thresholds):
    ...
    for label, subset in (("original_132", rows[:132]), ("extension_24", rows[132:156]), ("new_36", rows[156:])):
...
def selection_key(report):
    return (report["original_132"]["exact"] == 132, report["original_132"]["exact"] + report["extension_24"]["exact"] >= 155, report["exact"], report["valid"], -report["invented_authority"],
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "fresh baseline on frozen expanded 192-row development inventory",
        "checkpoints": [{k: r[k] for k in ("epoch", "cumulative_steps", "total", "valid", "exact",
            "invented_authority", "paraphrase_equal", "contrast_distinct", "parser_errors", "original_132", "extension_24", "new_36")}
