"""Bounded lower-rate continuation from the selected v9 adapter."""
...
from validation.event_graph_v9 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v9-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 1200, "seeds": [97],
        "learning_rate": 0.00001, "baseline_steps": 1440,
        "initialization": "selected v9 epoch 1 weights; optimizer reset",
...
def freeze(run):
    ...
    assert preflight['train']['count']==1200 and preflight['valid']['count']==132
    ...
    paths.append(ROOT/'gateway/tests/test_event_graph_v10.py')
...
def train(run, epoch):
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 600, "save_every": 1200,
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "reused v9 epoch 1 on identical development data and protocol",
