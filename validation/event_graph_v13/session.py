"""Bounded lower-rate continuation from the selected v11 adapter."""
...
from validation.event_graph_v11 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v11-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 720, "seeds": [127],
        "learning_rate": 0.00001, "baseline_steps": 3300,
        "initialization": "selected v11 epoch 1 weights; optimizer reset",
...
def freeze(run):
    ...
    paths.append(ROOT/'gateway/tests/test_event_graph_v13.py')
...
def train(run, epoch):
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 720, "save_every": 720,
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "fresh baseline generation on expanded development; same protocol, 156 rows",
