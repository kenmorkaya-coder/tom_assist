"""Bounded lower-rate continuation from the selected v13 adapter."""
...
from validation.event_graph_v13 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v13-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 360, "seeds": [139],
        "learning_rate": 0.000005, "baseline_steps": 4020,
        "initialization": "selected v13 epoch 1 weights; optimizer reset",
        "selection": "retain original 132 exact first; then total exact, valid, fewer invented authorities, paraphrase, contrast, earliest"}


def read_rows(split):
    return parent.read_rows(split)
...
def verify(run):
    ...
    assert prior.sha(PARENT/'freeze.json')==frozen['parent_freeze']
    parent.verify(PARENT);parent.adapter(PARENT,1)
...
def freeze(run):
    inherited=parent.verify(PARENT);parent.adapter(PARENT,1)
    preflight=json.loads((parent.HERE/'PREFLIGHT.json').read_text())
    assert preflight['train']['count']==1440 and preflight['valid']['count']==156
    assert max(v['max_tokens'] for v in preflight.values()) <= 4096
    paths=[p for p in HERE.rglob('*') if p.is_file() and p.suffix in ('.py','.md','.json','.jsonl')]
    paths.append(ROOT/'gateway/tests/test_event_graph_v14.py')
    names=['epoch-1/'+n for n in ('dev_report.json','dev_predictions.jsonl','adapter/adapters.safetensors','adapter/adapter_config.json')]
    prior.write_new(run/'freeze.json',{'created_unix':time.time(),'plan':PLAN,
        'files':{str(p.relative_to(ROOT)):prior.sha(p) for p in paths},
        'parent_freeze':prior.sha(PARENT/'freeze.json'),'thresholds':inherited['thresholds'],
        'parent_artifacts':{n:prior.sha(PARENT/n) for n in names},'development_records':156})


def reuse_baseline(run):
    frozen=verify(run);rows=read_rows('dev');sources={r['id']:r['source'] for r in rows}
    d=PARENT/'epoch-1';path=d/'dev_predictions.jsonl'
    predictions=[json.loads(x) for x in path.read_text().splitlines()]
    for p in predictions:
        try:g=parse_output(p['raw'],sources[p['id']])
    ...
    saved=json.loads((d/'dev_report.json').read_text());actual=development_score(rows,predictions,frozen['thresholds'])
    assert all(saved[k]==v for k,v in actual.items())
    assert saved['prediction_sha256']==prior.sha(path)
    assert saved['adapter_sha256']==prior.sha(parent.adapter(PARENT,1)/'adapters.safetensors')
    directory=stage(run,0);directory.mkdir(parents=True,exist_ok=True)
    with (directory/'dev_predictions.jsonl').open('x') as f:f.write(path.read_text())
    saved.update(epoch=0,baseline_reused=True,new_generations=0,source_report=str(d/'dev_report.json'))
    prior.write_new(directory/'dev_report.json',saved)


def adapter(run, epoch):
    if epoch == 0:
        return parent.adapter(PARENT,1)
...
def train(run, epoch):
    frozen = verify(run)
    if epoch != 1:
        raise ValueError("only block 1 can train")
    directory = stage(run, epoch)
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 360, "save_every": 360,
        "model": str(prior.MODEL), "data": str(parent.HERE / "training"),
    ...
    for split in ("train", "valid"):
        for line in (parent.HERE / "training" / f"{split}.jsonl").read_text().splitlines():
    ...
    prior.write_new(directory / "training_complete.json", {"time": time.time(),
        "iterations": config["iters"], "cumulative_steps": PLAN["baseline_steps"] + PLAN["steps_per_epoch"] * epoch,
...
def development_score(rows, predictions, thresholds):
    ...
    for label, subset in (("original_132", rows[:132]), ("extension_24", rows[132:])):
        ids={r['id'] for r in subset}
        score=prior.score_extraction(subset,[p for p in predictions if p['id'] in ids],thresholds)
        result[label]={k:score[k] for k in ('total','exact','valid','invented_authority')}
    return result
...
def evaluate(run, epoch):
    ...
    report.update(epoch=epoch, cumulative_steps=PLAN["baseline_steps"] + PLAN["steps_per_epoch"] * epoch,
...
def selection_key(report):
    return (report["original_132"]["exact"] == 132, report["exact"], report["valid"], -report["invented_authority"],
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "reused verified v13 checkpoint on identical 156-row development inventory",
        "checkpoints": [{k: r[k] for k in ("epoch", "cumulative_steps", "total", "valid", "exact",
            "invented_authority", "paraphrase_equal", "contrast_distinct", "parser_errors", "original_132", "extension_24")}
