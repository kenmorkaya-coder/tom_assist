"""Matched 96-update causal-coverage pilot; outputs belong on Passport."""
...
RUN = Path('/Volumes/My Passport for Mac/gemmatraining/event-graph-lora-v19-pilot-001')
PLAN = dict(updates_per_arm=96, seed=151, learning_rate=0.000005, baseline_steps=4380,
    arms=['control', 'coverage'], changed_records=8, optimizer='reset identically in both arms',
    variable='reverse causal roles in eight existing effect-first training examples',
    retention='all original132 exact; prior156 at least155 exact; dev192 exact at least191 and all192 valid',
    evidence='coverage must beat control and baseline on unique V18 diagnostic exact without losing source/target correctness on any original anchor; otherwise inconclusive or harmful',
    selection='retain4380 regardless; review pilot before further training or promotion',
    limitation='selected96-row pilot is not the full training mix; one seed does not establish robust improvement; exposed diagnostic is not held-out')


def training_rows(arm):
    assert arm in PLAN['arms']
    all_rows = old.read_rows('train')
    families = defaultdict(list)
    for row in all_rows[:1440]:
        if row['family'] != 'direction': families[row['family']].append(row)
    representative = []
    for n in range(48):
        names = sorted(families); name = names[n % len(names)]
        representative.append(families[name][n // len(names)])
    causal = [r for r in all_rows[1440:] if r['family'] == 'direction']
    rows = copy.deepcopy(representative + causal)
    if arm == 'coverage':
        for row in rows:
            if row['id'].startswith('v16-train-direction-') and row['variant'] == 'paraphrase' and int(row['id'].split('-')[-2]) % 2 == 0:
                roles = semantic_graph(row['graph'])['events'][0]['roles']
                source, target = roles['target'], roles['source']
                text = f'The cause of {target} is {source}.'
                g = Builder(text); src = g.entity(source); dst = g.entity(target)
                g.event('cause', roles={'source': src, 'target': dst}, modality='assertion')
                validate_graph(g.graph, text); row.update(source=text, graph=g.graph)
...
def verify(run):
    ...
    old.verify(V16); diag.verify(V18)
...
def freeze(run):
    ...
    files = [Path(__file__).resolve(), ROOT/'gateway/tests/test_event_graph_v19.py',
    ...
    frozen = {'created_unix':time.time(),'plan':PLAN,'files':{str(p):old.prior.sha(p) for p in files},
        'thresholds':old.verify(V16)['thresholds'],'training_ids':[r['id'] for r in training_rows('control')],
        'changed_ids':[a['id'] for a,b in zip(training_rows('control'),training_rows('coverage')) if a!=b]}
