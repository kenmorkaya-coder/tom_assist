"""Bounded lower-rate continuation from the selected v10 adapter."""
...
from validation.event_graph_v10 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v10-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 660, "seeds": [109],
        "learning_rate": 0.00001, "baseline_steps": 2640,
        "initialization": "selected v10 epoch 1 weights; optimizer reset",
        "selection": "exact, valid, fewer invented authorities, paraphrase, contrast, earliest"}


def read_rows(split):
    return parent.read_rows(split) if split != 'train' else [json.loads(x) for x in (HERE/'data/train.jsonl').read_text().splitlines()]
...
def freeze(run):
    ...
    assert preflight['train']['count']==1320 and preflight['valid']['count']==132
    ...
    paths.append(ROOT/'gateway/tests/test_event_graph_v11.py')
...
def train(run, epoch):
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 660, "save_every": 660,
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "reused v10 epoch 1 on identical development data and protocol",
