...
DATA_DIR = ROOT / "validation/event_graph_v3"
...
CONFIG = {"seed": 7, "train": True, "test": False, "fine_tune_type": "lora", "num_layers": 16,
          "batch_size": 1, "iters": 512, "val_batches": 16, "learning_rate": 0.0001,
          "steps_per_report": 8, "steps_per_eval": 128, "save_every": 128,
          "max_seq_length": 4096, "grad_checkpoint": True, "mask_prompt": True,
          "lora_parameters": {"rank": 16, "dropout": 0.0, "scale": 16.0, "keys": ["self_attn.q_proj", "self_attn.v_proj"]}}


def read_rows(split):
    return [json.loads(line) for line in (DATA_DIR / "data" / f"{split}.jsonl").read_text().splitlines()]
...
def freeze(run):
    ...
    files += [ROOT / "gateway/tests/test_event_graph_v3.py", ROOT / "gateway/tests/test_gemma_training_alignment.py", ROOT / "gateway/gemma_training_alignment.py", ROOT / "gateway/event_graph_extractor_v3.py", ROOT / "validation/event_graph_v1/build_corpus.py"]
    files += sorted(p for p in DATA_DIR.rglob("*") if p.is_file() and p.suffix in (".py", ".json", ".jsonl", ".md") and "__pycache__" not in p.parts)
...
def train(run):
    ...
    for name in ("train", "valid"):
        for line in (DATA_DIR / "training" / f"{name}.jsonl").read_text().splitlines():
            lengths.append(check_alignment(tokenizer, json.loads(line)["messages"])["total_tokens"])
    ...
    config = {**CONFIG_DEFAULTS, **frozen["training"], "model": str(MODEL), "data": str(DATA_DIR / "training"), "adapter_path": str(run / "adapter")}
    ...
    print("Loading pinned model for aligned supervised training", flush=True)
    model, native_tokenizer = load(str(MODEL))
    for name in ("train", "valid"):
        for line in (DATA_DIR / "training" / f"{name}.jsonl").read_text().splitlines():
            check_alignment(native_tokenizer, json.loads(line)["messages"])
    aligned_tokenizer = AlignedChatTokenizer(native_tokenizer)
    args = SimpleNamespace(**config)
    train_set, valid_set, _ = load_dataset(args, aligned_tokenizer)
    print("Native inference and supervised prefixes verified for every train/dev example", flush=True)
    train_model(args, model, train_set, valid_set, training_callback=None)
