"""Bounded lower-rate continuation from the selected v8 adapter."""
...
from validation.event_graph_v8 import session as parent
from gateway.gemma_training_alignment import AlignedChatTokenizer, check_alignment

HERE = Path(__file__).parent
PARENT = ROOT / ".tmp/event-graph-lora-v8-session-001"
PLAN = {"epochs": 1, "steps_per_epoch": 480, "seeds": [73],
        "learning_rate": 0.00001, "baseline_steps": 960,
        "initialization": "selected v8 epoch 1 weights; optimizer reset",
        "selection": "exact, valid, fewer invented authorities, paraphrase, contrast, earliest"}


def read_rows(split):
    return [json.loads(x) for x in (parent.HERE/'data'/f'{split}.jsonl').read_text().splitlines()]
...
def freeze(run):
    ...
    paths=[p for p in HERE.rglob('*') if p.is_file() and p.suffix in ('.py','.md','.json')]
    paths.append(ROOT/'gateway/tests/test_event_graph_v9.py')
    ...
    prior.write_new(run/'freeze.json',{'created_unix':time.time(),'plan':PLAN,
        ...
        'parent_artifacts':{n:prior.sha(PARENT/n) for n in names},'development_records':132})
...
def train(run, epoch):
    ...
    config = {**CONFIG_DEFAULTS, **prior.CONFIG, "seed": seed,
        ...
        "steps_per_report": 16, "steps_per_eval": 480, "save_every": 480,
...
def comparison(reports):
    ...
    return {"status": "DEVELOPMENT_COMPARISON_ONLY", "selected_epoch": best["epoch"],
        ...
        "reference": "reused v8 epoch 1 on identical development data and protocol",
